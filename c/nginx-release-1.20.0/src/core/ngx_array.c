...
ngx_array_create(ngx_pool_t *p, ngx_uint_t n, size_t size)
{
    ...
    /**
     * 数组初始化，并且分配内存空间给数组元素
     * PS:这边数组的数据结构和数组元素的存储分成了两次在pool上分配
     */
...
ngx_array_push(ngx_array_t *a)
{
    ...
    if (a->nelts == a->nalloc) {
            ...
            && p->d.last + a->size <= p->d.end)
        {
            ...
            // size不变（最大容量，即单个元素大小 * 最大元素数）
            // pool不变（因为分配新的内存块的时候，会循环读取pool->d.next链表上的缓存池）
            // 并且比较剩余空间大小，是否可以容纳新的内存块存储
            // nelts已用元素数量不变
        }
    }
