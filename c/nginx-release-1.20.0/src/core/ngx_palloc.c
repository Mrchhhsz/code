...
ngx_create_pool(size_t size, ngx_log_t *log)
{
     ...
     * ngx_pool_data_t p->d存放内存池的数据部分（适合小于p->max的内存块存储）
     * p->large 存放大内存块列表
     * p->cleanup存放可以被回调函数清理的内存块（该内存不一定会在内存池上面分配）
     */
...
ngx_reset_pool(ngx_pool_t *pool)
{
    ...
    // 循环重新设置内存池的data区域的 p->d.last;data区域数据并不擦除
...
ngx_palloc_small(ngx_pool_t *pool, size_t size, ngx_uint_t align)
{
     ...
     *
     * 这边的循环，实际上最多只有4次，ngx_palloc_block函数
     */
    do {
        m = p->d.last;
...
ngx_palloc_block(ngx_pool_t *pool, size_t size)
{
     ...
     * 子节点的ngx_pool_t数据结构中只会用到pool->d的结构，只保存数据，没添加一个子节点，p->d.failed就会+1，当添加超过4个子节点的时候，
 ...
 * 对于这个链表中每个节点所包含的资源如何去释放，是字说明的。这也就提供了非常大的灵活性。
 * 意味着，ngx_pool_t不仅仅可以管理内存，通过这个机制，也可以管理任何需要释放的资源，
 * 例如，关闭文件，或者删除文件等等的。下面我们看一下这个链表的每个节点的类型
 *
 * 一般分为两种请况：
 * 1.文件描述符
 * 2.外部定义回调函数可以来清理内存
