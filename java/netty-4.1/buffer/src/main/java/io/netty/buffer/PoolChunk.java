...
final class PoolChunk<T> implements PoolChunkMetric {
    ...
    // 真正分配的内存，如果是堆内的话就是字节数组，否则就是直接缓冲区DirectByteBuffer，这个是真正操作分配的内存
    // 其他的一些都是逻辑上分配内存
    final T memory;
    // 是否池化
    ...
    // 页位移，也就是pageSize=1 <<< pageShifts，8k就是13
    ...
    boolean allocate(PooledByteBuf<T> buf, int reqCapacity, int sizeIdx, PoolThreadCache cache) {
        ...
        if (sizeIdx <= arena.smallMaxSizeIdx) {
            // small
            handle = allocateSubpage(sizeIdx); // 子页来分
            if (handle < 0) {
                return false;
            }
            ...
            handle = allocateRun(runSize); // run来分
            if (handle < 0) {
                return false;
            }
        }
    ...
    private long allocateSubpage(int sizeIdx) {
        ...
        // 获取子页数组的头结点，可以是tiny数组也可以是small数组的
