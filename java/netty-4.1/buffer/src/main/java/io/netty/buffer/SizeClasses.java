...
abstract class SizeClasses implements SizeClassesMetric {
    ...
    /**
     * SizeClasses初始化后，将计算chunkSize(内存池每次向操作系统申请内存块大小) 范围内每个size的值
     * 保存到sizeClasses字段中.
     * sizeClasses是一个表格，共有7列
     * 1. index: 内存块size的索引
     * 2. log2Group: 内存块分组，用于计算对应的size
     * 3. log2Delta: 增量大小的log2值，用于计算对应的size
     * 4. nDelta: 增量乘数，用于计算对应的size
     * 5. isMultiPageSize: 表示size是否为page的倍数
     * 6. isSubPage: 表示是否为一个subPage类型
     * 7. log2DeltaLookup: 如果size存在的位图中，记录其中log2Delta，未使用
     */
    ...
    private int sizeClasses() {
        ...
        int ndeltaLimit = 1 << LOG2_SIZE_CLASS_GROUP; // 1 << 2 = 4 即内存块size以4个为一组进行分组
    ...
    // 通过size2SizeIdx计算出 size在sizeClasses中的位置为 组 << LOG2_SIZE_CLASS_GROUP + 在组中的位置 即 group + mod;
    // 该方法参数就是上面说的计算值
    //
    @Override
    ...
    // 计算size在sizeClasses中的位置
    @Override
    public int size2SizeIdx(int size) {
        if (size == 0) {
            return 0;
        }
        // 大于chunkSize 返回nSizes 代表申请的是Huge内存块
        if (size > chunkSize) {
            return nSizes;
        }
        // 不使用sizeClasses表格，直接申请内存大小转换为directMemoryCacheAlignment的倍数
        if (directMemoryCacheAlignment > 0) {
            size = alignSize(size);
        }
        // SizeClasses 将一部分较小的size与对应index记录在size2idxTab作为位图，这里直接查询size2idxTab
        // 避免重复计算
        ...
        // 对申请内存大小进行log2的向上取整，就是每组最后一个内存块size
        // -1是为了避免申请内存大小刚好等于2的指数次幂
        int x = log2((size << 1) - 1);
        // 当前在第几组，第一组最后一位的size最大是 64 1000000
