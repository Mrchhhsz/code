 ...
 * for a given segment.
 *
 * @param _dir The directory in which log segments are created. 存储每个logSegment对应的日志文件和索引文件
...
class Log(@volatile private var _dir: File,
  ...
  /* A lock that guards all modifications to the log 可能有多个Handler线程同时操作，需要加锁 */
  ...
  /* the actual segments of the log 使用跳表存储LogSegment
  * 文件名作为key, value就是这个LogSegment对象，根据offset快速定位到对应的logSegment */
   ...
   * 将消息集分派到活跃的segement,并且不设置偏移量或分区 leader纪元（该消息是follower节点同步leader节点的消息）
   *
   * @param records The records to append
   * @throws KafkaStorageException If the append fails due to an I/O error.
   * @return Information about the appended messages including the first and last offset.
      ...
      lock synchronized {
        maybeHandleIOException(s"Error while appending records to $topicPartition in dir ${dir.getParent}") {
          ...
          if (validateAndAssignOffsets) {
            ...
            // 每条msg都会有一个对应的时间戳记录，producer端可以设置这个字段message.timestamp.type来选择timestamp的类型
            // 默认为创建时间，如果设置为日志追加时间
          ...
          maybeDuplicate match {
            ...
            case None =>
              ...
              // 当没有磁盘的消息累积到flushInterval时，做一次flush
              // todo 根据条件判断，把内存中的数据写入到磁盘中，这一块其实是不会走的
  ...
  private def deleteRetentionMsBreachedSegments(): Int = {
    ...
    // 当前时间 - segment最后一次修改时间 > 配置文件中配置的时间，则删除
  ...
  def roll(expectedNextOffset: Option[Long] = None): LogSegment = {
    maybeHandleIOException(s"Error while rolling log segment for $topicPartition in dir ${dir.getParent}") {
      ...
      lock synchronized {
        ...
        // 基于LEO值生成一个新的文件，新日志文件名是 LEO.log
   ...
   * 将recoveryPoint~logEndOffset之间的数据都刷到磁盘，recoveryPoint之前的数据是已经刷到磁盘的
   */
  ...
  def flush(offset: Long): Unit = {
    maybeHandleIOException(s"Error while flushing log for $topicPartition in dir ${dir.getParent} with offset $offset") {
      if (offset > this.recoveryPoint) {
        ...
        // 通过推segments跳表来查找recoveryPoint和offset之间的logSegment
        logSegments(this.recoveryPoint, offset).foreach(_.flush())
        lock synchronized {
          checkIfMemoryMappedBufferClosed()
          if (offset > this.recoveryPoint) {
            // 将recoverPoint指针指向新刷新到的结束的位置
            this.recoveryPoint = offset
