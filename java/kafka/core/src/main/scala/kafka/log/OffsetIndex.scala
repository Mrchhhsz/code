...
class OffsetIndex(_file: File, baseOffset: Long, maxIndexSize: Int = -1, writable: Boolean = true)
  ...
  // 将offset和物理地址（position）封装成offsetPosition对象并返回
  override protected def parseEntry(buffer: ByteBuffer, n: Int): OffsetPosition = {
    // relativeOffset() 方法和physical() 方法是获取索引项内容的辅助方法，分别实现了读取索引项中的相对offset和索引项
    // 中的物理地址（position）的功能
  ...
  def append(offset: Long, position: Int): Unit = {
    inLock(lock) {
      ...
      if (_entries == 0 || offset > _lastOffset) {
        ...
        // todo position:是物理地址，表示该消息在磁盘文件中的哪一块位置
        mmap.putInt(position)
        // 结论，写索引的时候会记录2个位置信息，一个是offset,就是我们所说的偏移量，它是逻辑上的，还有一个是position,它是物理的，
        // 记录下消息在磁盘中的位置
        _entries += 1
        _lastOffset = offset
