 ...
 * 日志管理子系统入口，负责日志的创建，恢复和删除
 * 读写操作委托给各个日志实例
 */
@threadsafe
class LogManager(logDirs: Seq[File], // 日志目录 通过log.dirs来指定
  ...
  // 尝试对每个log目录在文件系统层面加锁，这里加的是进程锁
  private val dirLocks = lockLogDirs(liveLogDirs)
  /**
   * 遍历为每个log目录创建一个操作其名下recovery-point-offset-checkpoint 文件的OffsetCheckPoint对象，
   * 非建立映射关系
   */
    ...
    for (dir <- liveLogDirs) {
      ...
      try {
        ...
        try {
          ...
          // 文件为 recovery-point-offset-checkpoint
        ...
        val jobsForDir = logsToLoad.map { logDir =>
          val runnable: Runnable = () => {
            try {
              ...
              // todo 每个日志子目录生成一个job加载log,并且根据读取的recoveryPoints,logStartOffset进行加载
  ...
  def cleanupLogs(): Unit = {
    ...
    val deletableLogs = {
      if (cleaner != null) {
        ...
        // 如果配置文件的 cleanup.policy不为delete，则不会删除
...
object LogManager {
  ...
  def apply(config: KafkaConfig,
    ...
    // 配置了kafka存储的目录参数： log.dirs，在生产环境中通过会对应多个目录
