 ...
 *
 * LogSegment是一个逻辑上的概念，一个LogSegment对应磁盘上的一个日志文件和一个索引文件，日志文件记录消息，索引文件保存消息的索引
 *
 * 每个日志文件都对应一个索引文件，使用稀疏索引，并不是每条消息一个索引项
 *
...
class LogSegment private[log] (val log: FileRecords,
  ...
  // todo 是否需要新建LogSegment
  def shouldRoll(rollParams: RollParams): Boolean = {
    val reachedRollMs = timeWaitedForRoll(rollParams.now, rollParams.maxTimestampInMessages) > rollParams.maxSegmentMs - rollJitterMs
    // 如果一个segment大小超过1个G,就需要新建一个segment
    size > rollParams.maxSegmentBytes - rollParams.messagesSize ||
    // 距离上次创建日志分段的时间达到了一定的阈值，（log.roll.hours） 7天，并且数据文件有数据
      (size > 0 && reachedRollMs) ||
    // 索引文件满了（默认10M）log.index.size.max.bytes
    // 时间索引文件满了（默认10M）
    // 最大的offset,其相对偏移量超过了正整数的阈值，追加的消息的偏移量与当前日志分段的偏移量之间的差值大于Integer.MAX_VALUE
    // 索引文件中的每个索引占8字节，4个字节是相对偏移量,4个字节是日志在文件中的物理地址
  ...
  def append(largestOffset: Long,
             ...
             records: MemoryRecords): Unit = { // MemoryRecords是Kafka中Record在内存中的实现方式
    // 判断消息的大小
    if (records.sizeInBytes > 0) {
      // 检查是否满足添加索引项的条件
      trace(s"Inserting ${records.sizeInBytes} bytes at end offset $largestOffset at position ${log.sizeInBytes} " +
            s"with largest timestamp $largestTimestamp at shallow offset $shallowOffsetOfMaxTimestamp")
      // 获取FileRecord文件的末尾，他就是本次消息要写入的物理地址
      ...
      // 调用FileRecords的append方法添加records
      ...
      // 当已写入字节数超过了4kB之后，append方法会调用索引对象的append方法新增索引项，同时清空已写入字节数
      // todo 这里并不是来一条数据就写一条索引，而是达到一定的条件才写一次索引，我们管它叫稀疏索引
      // indexIntervalBytes默认是4096字节
      // 每次写了4096个字节的消息就更新一次索引
      if (bytesSinceLastIndexEntry > indexIntervalBytes) {
        // 添加索引， 写索引的方法
        offsetIndex.append(largestOffset, physicalPosition)
        // 时间索引，一般用不到
        timeIndex.maybeAppend(maxTimestampSoFar, offsetOfMaxTimestampSoFar)
        // 成功添加索引后，byteSinceLastIndexEntry重置为0
        bytesSinceLastIndexEntry = 0
      }
      // 更新bytesSinceLastIndexEntry
      // 添加消息大小到bytesSinceLastIndexEntry变量中
  ...
  def recover(producerStateManager: ProducerStateManager, leaderEpochCache: Option[LeaderEpochFileCache] = None): Int = {
    ...
    offsetIndex.trimToValidSize() // 对索引文件进行相应截断
