 ...
 * topicPartition topic名称和分区编号
 * localBrokerId 当前broker的id，
 * logManager 当前broker上的LogManager对象
 *
 * Partition中的功能划分：
 * 1.获取（或创建）Replica
 * 2.副本的Leader/Follower角色切换 makeLeader(),makeFollow()
 * 3.ISR集合管理
 * 4.调用日志存储子系统完成消息写入
 * 5.检测HW的位置
 */
class Partition(val topicPartition: TopicPartition,
  ...
  /**
    ...
    * 该方法假定当前副本已经创建
    * 在下面两种请况下创建future replica：
   * 1）当前副本不在所给的日志目录下
   * 2）future replica不存在 (根据future log是否存在来判断replica是否存在)
    */
  ...
  /**
   * 该方法是用来创建replica，log存在即replica存在
   * @param isNew
   ...
   * 删除分区
   * 注意：删除分区不会删除他底层的日志，在已经删除分区后，他的日志会被 replicaManager删除
   */
  def delete(): Unit = {
   ...
   * 将本地副本变成leader节点，重置远程副本的leo，设置新的leader节点和同步副本
   */
   ...
   * 根据最后一次fetchrequest的leader更新follower的状态，
   *
  ...
  private def maybeExpandIsr(followerReplica: Replica, followerFetchTimeMs: Long): Unit = {
    // 如果inflight == false且当前的ISR中不存该follower节点，则表示可以添加副本到ISR集合中
   ...
   *
   * HW被所有的ISR中最小的leo决定，如果一个副本被考虑赶上，但是他的LEO小于HW，我们需要等待他赶上（消息同步）再去提高HW
  ...
  def getOutOfSyncReplicas(maxLagMs: Long): Set[Int] = {
    ...
    if (!current.isInflight) {
      // todo 获取在ISR列表中排除leaderReplica的其他请求
      val candidateReplicaIds = current.isr - localBrokerId
      val currentTimeMs = time.milliseconds()
      val leaderEndOffset = localLogOrException.logEndOffset
      // todo 过滤延迟的Replica
      // todo 条件time.milliseconds - r.lastCaughtUpTimeMs) > maxLagMs(默认10s)
   ...
   * 在给定的偏移量处截断当前分区的本地日志和检查日志恢复的断点到所给的offset
    *
