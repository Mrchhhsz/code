...
/**
 * @param brokerId 标识该副本所在的broker id，根据当前broker的id和Replica.brokerId可以判断当前副本是远程副本还是本地副本
 * @param topicPartition
 */
class Replica(val brokerId: Int, val topicPartition: TopicPartition) extends Logging {
  // the log end offset value, kept in all replicas;
  // for local replica it is the log's end offset, for remote replicas its value is only updated by follower fetch
  // 所有的副本都持有leo，如果是本地副本，标识日志的最新消息偏移量，如果是远程副本的，只能通过远程副本的fetch请求更新
  ...
  // This is used to determine the lastCaughtUpTimeMs of the follower
  // leader节点最后一次接收到follower节点的FetchRequest的时间
