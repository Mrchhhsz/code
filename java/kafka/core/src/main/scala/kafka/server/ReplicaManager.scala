...
class ReplicaManager(val config: KafkaConfig,
  ...
  def appendRecords(timeout: Long, // 请求处理超时时间
    ...
    if (isValidRequiredAcks(requiredAcks)) {
      ...
      if (delayedProduceRequestRequired(requiredAcks, entriesPerPartition, localProduceResults)) {
        ...
        // 尝试去完成delayedProduce，没过期将其添加到delayedProducePurgatory中管理
  ...
  def fetchMessages(timeout: Long,
    ...
    // todo 请求不想等待 or 请求消息为空 or 读取的总字节数超过了最小积攒字节数 or 存在读取错误 or 方向epoch不合法
    // 此时直接生成结果给回调函数处理
  ...
  private def makeFollowers(controllerId: Int,
    ...
    try {
        ...
        try {
          metadataCache.getAliveBrokers.find(_.id == newLeaderBrokerId) match {
            // Only change partition state when the leader is available
              // 如果leader是可用的，则仅仅改变分区状态，检测新leader所在的broker是否存活
            case Some(_) =>
              // 将分区的Local Replica切换为Follower副本
            ...
            case None =>
              ...
              // 即使是leader副本所在的Broker不可用，也要创建Local Replica，主要是为了在checkPoint文件中记录
              // 此分区的HW
      ...
      if (isShuttingDown.get()) {
        ...
        // todo 添加了followPartition向leader Partition 拉取数据的任务
