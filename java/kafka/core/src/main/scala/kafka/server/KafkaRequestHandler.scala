...
class KafkaRequestHandler(id: Int,
  ...
  def run(): Unit = {
    while (!stopped) {
      ...
      // 从requestChannel的requestQueue获取请求 Request对象
      ...
      req match {
        ...
        case request: RequestChannel.Request =>
          try {
            ...
            // KafkaApis类中实现了处理请求的逻辑，并且负责把response写回对应的requestChannel中的responseQueue中，唤醒Processor
            apis.handle(request)
          } catch {
...
class KafkaRequestHandlerPool(val brokerId: Int,
  ...
  // todo 创建一个定长为8的数组存储KafkaRequestHandler线程，numThreads: num.io.threads 默认值为8
  ...
  def shutdown(): Unit = synchronized {
    ...
    for (handler <- runnables)
      handler.awaitShutdown() // 阻塞等待所有kafka线程结束
