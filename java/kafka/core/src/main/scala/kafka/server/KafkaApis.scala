...
class KafkaApis(val requestChannel: RequestChannel,
    ...
    try {
      ...
      request.header.apiKey match {
        ...
        case ApiKeys.PRODUCE => handleProduceRequest(request)
          // todo 接收follower拉取数据的请求，同时也接收consumer消费数据fetch的请求
  ...
  def handleFetchRequest(request: RequestChannel.Request): Unit = {
    ...
    // todo fetch 请求来自于follower
    if (fetchRequest.isFromFollower) {
      // The follower must have ClusterAction on ClusterResource in order to fetch partition data.
      // 认证成功，判断请求的每个分区是否存在，若存在则将分区对应的请求加入interesting中
      // 否则则构造错误响应加入erroneous
      ...
      } else { // todo 认证失败，对所有分区都构造错误响应加入erroneous
      ...
      // todo fetch请求来自于客户端（消费者），和之前处理一样，认证失败或者分区不存在则构造错误响应
    ...
    if (interesting.isEmpty)
      ...
      // todo 调用ReplicaManager.fetchMessages方法对interesting请求进行处理
      replicaManager.fetchMessages(
        fetchRequest.maxWait.toLong, // 最大等待时间
        fetchRequest.replicaId,      // 副本ID,客户端为Consumer 则为-1
        fetchMinBytes,               // 请求中的最小字节数
        fetchMaxBytes,               // 请求中最大字节数
        versionId <= 2,              // 判断API版本以兼容旧版本请求
      ...
      if (topicMetadata.headOption.isEmpty) {
        ...
        def createResponse(requestThrottleMs: Int): AbstractResponse = {
          ...
          val responseBody = if (topicMetadata.head.errorCode != Errors.NONE.code) {
            ...
            // todo 获取该分区leader partition所在的节点
