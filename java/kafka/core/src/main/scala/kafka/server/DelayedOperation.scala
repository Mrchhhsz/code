 ...
 */
abstract class DelayedOperation(override val delayMs: Long, // 延迟时间
  ...
  def forceComplete(): Boolean = {
    // 原子操作，确保onComplete只执行一次
    if (completed.compareAndSet(false, true)) {
      // cancel the timeout timer
      // 调用timerTask的cancel，将任务从时间轮中删除
      cancel()
      // 调用延迟操作的真正逻辑
      onComplete()
      true
   ...
   * 到期后被SystemTimer的taskExecutor执行，调用forceComplete
   */
...
final class DelayedOperationPurgatory[T <: DelayedOperation](purgatoryName: String,
  ...
  private class WatcherList {
    // pool底层是ConcurrentHashMap， value Watchers是DelayedOperation的集合， key是watchers中的延迟操作关心的对象
  ...
  // 该线程负责推进时间轮表针，和清理过期的delayedOperation
  ...
  private class Watchers(val key: Any) {
    ...
    def tryCompleteWatched(): Int = {
      ...
      while (iter.hasNext) {
        val curr = iter.next()
        // 如果已经完成，将该operation从队列中移除
        if (curr.isCompleted) {
          // another thread has completed this operation, just remove it
          iter.remove()
        } else if (curr.safeTryComplete()) { // 尝试完成
          iter.remove()
          completed += 1
        }
      }

      // 如果operations队列中没有元素了，将其从WatchersForKey中移除
