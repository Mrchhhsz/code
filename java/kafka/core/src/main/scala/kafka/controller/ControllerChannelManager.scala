...
class ControllerChannelManager(controllerContext: ControllerContext,
  ...
  // 用于管理集群中每个broker对应的ControllerBrokerStateInfo
  ...
  def startup() = {
    // 为broker创建对应的ControllerBrokerStateInfo
...
class RequestSendThread(val controllerId: Int,
  ...
  /**
   * 接收三种请求，replicaAndISR请求，停止Replica请求，更新元数据请求
   */
  override def doWork(): Unit = {
    ...
    try {
      ...
      while (isRunning && !isSendSuccessful) {
        ...
        // 当broker拓机后，会触发zookeeper的监听器调用removeBroker()方法将当前线程停止，在停止前会一直尝试重试
        try {
          if (!brokerReady()) {
...
/** 为了提高Controller Leader与集群中其他Broker的通信效率，KafkaController使用该组件实现批量发送请求的功能 */
...
abstract class AbstractControllerBrokerRequestBatch(config: KafkaConfig,
  ...
  // 该方法会向leaderAndIsrRequestMap集合中添加待发送的LeaderAndIsrRequest所需的数据，同时会
  // 调用addUpdateMetadataRequestForBrokers()方法准备向集合中所有可用的Broker发送UpdateMetadataRequest
  def addLeaderAndIsrRequestForBrokers(brokerIds: Seq[Int], // 第一个参数指定了接收LeaderAndIsrRequest的broker集合
  ...
  /** Send UpdateMetadataRequest to the given brokers for the given partitions and partitions that are being deleted */
    // 发送 updateMetadataRequest到被给的partitions的brokers，
