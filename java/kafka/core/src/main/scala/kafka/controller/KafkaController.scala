...
class KafkaController(val config: KafkaConfig,
   ...
   */
  def startup() = {
    // todo 注册zk状态变更监听器，他是用于监听zk和broker的会话过期的
    ...
    // todo 将startup事件加入到event queue中
    eventManager.put(Startup)
    // todo 启动eventManager 来处理event queue中的任务
  ...
  private def onControllerFailover(): Unit = {
    ...
    // todo 注册各种监听器
    // brokerChangeHandler 为Broker数量Zookeeper监听器
  ...
  private def elect(): Unit = {
    // todo 获取当前Controller所在broker的序号，如果Controller不存在，就显示标记为-1
    activeControllerId = zkClient.getControllerId.getOrElse(-1)
    /*
     * We can get here during the initial startup and the handleDeleted ZK callback. Because of the potential race condition,
     * it's possible that the controller has already been elected when we get here. This check will prevent the following
     * createEphemeralPath method from getting into an infinite loop if this broker is already the controller.
     * todo 如果controller已经选出来了，就返回
     */
...
class BrokerChangeHandler(eventManager: ControllerEventManager) extends ZNodeChildChangeHandler {
  ...
  override def handleChildChange(): Unit = {
    // todo 添加BrokerChange 事件到LinkedBlockingQueue[ControllerEvent] 队列中
