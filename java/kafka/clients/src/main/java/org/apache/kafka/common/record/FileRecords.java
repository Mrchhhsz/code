...
public class FileRecords extends AbstractRecords implements Closeable {
    // 表示当前Records是否为日志文件的分片
    private final boolean isSlice;
    //start和end表示分片的起始位置和结束位置
    ...
    // mutable state
    // 大小，单位是字节，可能有多个Handler线程并发向同一个分区写入消息，使用原子类
    ...
    FileRecords(File file,
        ...
        if (isSlice) {
            ...
            // 如果当前对象不是一个分片，更新文件的指针到文件的最后，将文件的地址设置到这个文件的最后一个字节
    ...
    public int append(MemoryRecords records) throws IOException {
        ...
        int written = records.writeFullyTo(channel); // 写文件
        size.getAndAdd(written); // 修改大小
        return written;
    }

    /**
     * Commit all written data to the physical disk
     * 写消息到磁盘文件，采用FileChannel来实现，处于性能方面的考虑，操作系统会将数据缓存再内存中（基于OS Cache）
     * 所以无法保证写入到FileChannel里的数据一定会及时写到磁盘中，要保证这一点，需要调用force方法
     */
