...
public class Sender implements Runnable {
    ...
    void runOnce() {
        ...
        // 实现数据请求发送出去，并且会接收服务端响应，处理响应数据
        client.poll(pollTimeout, currentTimeMs);
    }

    private long sendProducerData(long now) {
        // todo 获取元数据
        // 因为这里采用场景驱动的方式，由于代码第一次进来，还没有获取到元数据，所以这个cluster对象中是没有元数据的
        // 如果是第二次进来，这个时候就应该有对应的元数据了
        Cluster cluster = metadata.fetch();
        // get the list of partitions with data ready to send

        // todo 判断哪些partition有消息可以发送，获取到这个partition的leaderPartition对应的broker主机获取当前准备发送的partitions
        // 目前消息已经封装在不同分区队列中的批次中，判断哪些批次可以把数据发送出去
        RecordAccumulator.ReadyCheckResult result = this.accumulator.ready(cluster, now);

        // if there are any partitions whose leaders are not known yet, force metadata update
        // todo 如果有些partition没有leader信息，根性metadata
        ...
        while (iter.hasNext()) {
            Node node = iter.next();
            // todo 检查与要发送数据的主机网络是否建立好，去掉哪些不能发送信息的节点
            if (!this.client.ready(node, now)) {
                // 如果网络没有建立好，这里返回的就是false，
                // 移除这些主机
                iter.remove();
        ...
        // todo 有可能要发送的partition有很多个，这些partition的leader分区可能在同一台节点上

        // 获取要发送的records，如果网络没有建立好，这块代码也是不会执行的
        Map<Integer, List<ProducerBatch>> batches = this.accumulator.drain(cluster, result.readyNodes, this.maxRequestSize, now);
        addToInflightBatches(batches);
        // 保证顺序发送 也就是该参数max.in.flight.requests.per.connection = 1  当某个消息发送失败，使用重试机制会导致无序，这个参数可以解决
