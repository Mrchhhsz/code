...
public class NetworkClient implements KafkaClient {
    ...
    public boolean isReady(Node node, long now) {
         ...
         *
         * 条件2：canSendRequest(node.isString(), now)
    ...
    private boolean canSendRequest(String node, long now) {
        /**
         * todo : connectionStates.isReady(node,now)
    ...
    private void doSend(ClientRequest clientRequest, boolean isInternalRequest, long now, AbstractRequest request) {
        ...
        // todo 几区clientRequest构建InFlightRequest对象
        ...
        // todo 把send请求加入队列等待虽有的poll方法把它发送出去
    ...
    public List<ClientResponse> poll(long timeout, long now) {
        ...
        // todo 封装了一个要拉取元数据请求，目前并没有把请求发送出去，客户端缓存了这些请求
        ...
        // todo 处理响应，响应里面就有我们要的元数据
        handleCompletedReceives(responses, updatedNow);
        // 处理关闭的响应
        handleDisconnections(responses, updatedNow);
        // 处理连接状态变化
        handleConnections();
        // 处理handleConnections方法中标记需要获取支持的api版本信息的节点
    ...
    private void handleCompletedReceives(List<ClientResponse> responses, long now) {
        // todo 从completeReceives中遍历所有接收到的响应，completedReceives中的信息是在上一层的selector.poll中添加进去的
        for (NetworkReceive receive : this.selector.completedReceives()) {
            // 获取响应的节点ID
            String source = receive.source();
            // 从inFlightRequest中获取缓存的request对象
    ...
    class DefaultMetadataUpdater implements MetadataUpdater {
        ...
        public void handleSuccessfulResponse(RequestHeader requestHeader, long now, MetadataResponse response) {
            ...
            if (response.brokers().isEmpty()) {
                ...
                // todo 如果集群的元数据信息存在，更新metadata集群的元数据信息
