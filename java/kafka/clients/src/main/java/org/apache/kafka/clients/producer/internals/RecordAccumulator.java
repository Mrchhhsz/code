...
public final class RecordAccumulator {
    ...
    public RecordAppendResult append(TopicPartition tp,
        ...
        try {
            ...
            // todo 先根据分区获取该消息所属的队列中，
            //          如果有已存在的队列，那么我们就使用存在对了
            //          如果队列不存在，
            Deque<ProducerBatch> dq = getOrCreateDeque(tp);
            // 加锁
            // 假设有三个线程来了，都是需要把数据写往同一个分区中相同的队列
            synchronized (dq) {
                if (closed)
                    throw new KafkaException("Producer closed while send in progress");
                // todo 尝试向队列里面的批次中添加数据
                // 目前是只有了队列，数据是存储在批次中，（批次对象是需要分配内存的）
                // 目前还没有分配内存，所以添加数据会失败
            ...
            /**
             * todo 计算一个批次的大小
             * 在消息大小和批次大小之间取一个最大值，用这个值作为当前这个批次的大小
             * 有可能一个消息比设定好的大小还要大
             * 默认一个批次的大小是16k
             * note: 如果我们发送数据的时候，我们的消息大小超过了16k,那么消息就是一条一条的发送出去的
    ...
    public ReadyCheckResult ready(Cluster cluster, long nowMs) {
        ...
        // exhausted记录是否有其他的线程在等待释放空间，如果exhausted为true，说明内存池中的内存不够了
        // exhausted 它会作为一个是否要把该批次发送出去的条件
            ...
            synchronized (deque) {
                ...
                if (batch != null) {
                    ...
                    if (leader == null) {
                        ...
                        // backingOff: 是否允许重试的标志
                        boolean backingOff = batch.attempts() > 0 && waitedTimeMs < retryBackoffMs;
                        /**
                         * 使用场景驱动的方式去分析，第一次发送数据，之前也没有消息发送过，也没有重试这一说
                         * backingOff 应该为false
                         *
                         * 还需要等待多久timeToWaitMs = lingerMs 默认就是0，表示不需要等待
                         * 如果默认是0的话，就是过来一条消息就发送一条消息，很明显不合适的
                         * 所以我们发送数据的时候需要去设置这个参数
                         */
                        ...
                        /**
                         * full: 如果一个批次写满了，无论时间有没有到，都需要把消息发送出去
                         * expired 时间到了，不管批次数据是否写满，也需要把消息发送出去
                         * exhausted: 内存不够了，也需要把消息发送出去，最后释放内存
                         * close/flushInProgress ： producer客户端线程关闭，也需要把消息发送出去
                         */
