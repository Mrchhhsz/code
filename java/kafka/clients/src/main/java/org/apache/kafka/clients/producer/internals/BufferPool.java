...
public class BufferPool {
    ...
    public ByteBuffer allocate(int size, long maxTimeToBlockMs) throws InterruptedException {
        ...
        try {
            ...
            // todo poolableSize 就是一个批次的大小，默认就是16KB
            // 如果size我们要申请的批次大小正好等于一个批次默认的大小，并且内存池不为空
            // 代码第一次进来，内存池中没有内存的，获取不到
            ...
            if (this.nonPooledAvailableMemory + freeListSize >= size) {
                ...
                try {
                    ...
                    // 如果分配的内存的大小，还是没有达到我需要申请的内存大小，内存池就会一直不断的一点一点的去分配，等待别人释放内存
    ...
    private void freeUp(int size) {
        // 当申请内存大于nonPooledAvailableMemory时，从tree这个byteBuffer列表循环，
        // 将free Dequeue<ByteBuffer>中的元素poll出来，并且将其容量赋值给nonPooledAvailableMemory
