...
public class KafkaProducer<K, V> implements Producer<K, V> {
    ...
    // 消息的醉倒长度， max.request.size 配置，包含消息头，序列化之后的key和value
    ...
    KafkaProducer(ProducerConfig config,
        ...
        try {
            ...
            // todo 创建一个线程，里面传进去了一个sender对象
            // 内部包含了业务代码
            // 实现吧业务与线程进行隔离，这样会显得清晰点
    ...
    private Future<RecordMetadata> doSend(ProducerRecord<K, V> record, Callback callback) {
        ...
        try {
            ...
            // todo 根据分区器选择消息应该发送的分区
            // 根据元数据信息可以计算出消息应该发送到topic的哪一个分区
            ...
            // todo 判断要发送的消息有没有超过消息大小的最大值
            // KafkaProducer初始化的时候，指定了一个参数，代表的是producer最大能发送的是一条消息能有多大，默认是1M,需要修改
            ...
            // todo 给每一条消息都绑定他的回调函数，因为我们使用的是异步的方式发送的消息
            Callback interceptCallback = new InterceptorCallback<>(callback, this.interceptors, tp);

            if (transactionManager != null && transactionManager.isTransactional()) {
                transactionManager.failIfNotReadyForSend();
            }
            // todo 把消息放入Accumulator(32M的一个内存)
    ...
    private ClusterAndWaitTime waitOnMetadata(String topic, Integer partition, long nowMs, long maxWaitMs) throws InterruptedException {
        ...
        do {
            ...
            // todo 获取当前元数据的版本
            // 在Producer管理元数据时候，对于他来说元数据是没有版本号的，
            // 每次成功更新元数据，都会递增这个版本号
            // todo 把needUpdate标识赋值为true
            ...
            // todo 获取一下集群元数据的信息 （上面可能成功更新了集群元数据信息了）
            cluster = metadata.fetch();
            // todo 计算一下，拉取元数据已经花了多少时间
            elapsed = time.milliseconds() - nowMs;
            // todo 如果花的时间大于最大等待时间，那么就报错
