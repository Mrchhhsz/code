...
/**
 * {@link EventProcessor} 等待特定的序号可用，序号不可用的时候，实现自己的等待策略
 * 当依赖的序列号的序号大于需要消费的序号时，就可以消费了
 *
 * 1.对于直接与生产者相连的消费者来讲，依赖的序列就是生产者的序列
 * 2.对于其他的消费来讲，依赖的序列就是它的所有直接前驱消费者的序列
 *
...
{
    /**
     * {@link EventProcessor}等待特定的序号可用
     *
     * 方法有可能返回一个小于Sequence的值，依赖具体的策略实现
     * 这种情况的常见用途是发出超时信号，任何使用等待策略获取超时信号的事件处理器都应该显示处理这种情况
     *
     * 如果实现自己的等待策略，那么一定不能抛出任何声明之外的异常；否则可能导致严重错误，他可能导致数据/信号丢失
     ...
     *                          但是如果要实现生产者与消费者的等待通知协议时，需要cursor对象作为锁对象
     * @param dependentSequence on which to wait.
     *                          在这个{@link Sequence}上等待，知道它大于期望的序号
     * @param barrier           the processor is waiting on.
     *                          事件处理器关联的序列屏蔽，一个{@link EventProcessor}只关联一个SequenceBarrier
     * @return the sequence that is available which may be greater than the requested sequence.
     * 返回的是依赖的Sequence中的最大序号，可能大于期望的序号
