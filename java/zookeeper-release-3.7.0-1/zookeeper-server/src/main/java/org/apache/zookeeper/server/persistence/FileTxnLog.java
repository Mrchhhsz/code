...
public class FileTxnLog implements TxnLog, Closeable {
    ...
    public synchronized boolean append(TxnHeader hdr, Record txn, TxnDigest digest) throws IOException {
        ...
        if (hdr.getZxid() <= lastZxidSeen) {
            ...
            // 如果 hdr的zxid大于最大的zxid，将最大zxid设置为hdr的zxid
            lastZxidSeen = hdr.getZxid();
        }
        /**
         * 如果日志流为空，初始化写数据相关的流和{@link FileHeader}，并序列化fileheader到指定文件
         */
        if (logStream == null) {
        ...
        // 使用byte[]来更新当前的checksum
        crc.update(buf, 0, buf.length);
        // 写long类型数据
        oa.writeLong(crc.getValue(), "txnEntryCRC");
        // 将序列化的事务记录写入 OutputArchive
