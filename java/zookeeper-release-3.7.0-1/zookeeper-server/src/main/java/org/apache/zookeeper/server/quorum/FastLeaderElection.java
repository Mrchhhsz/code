            ...
            public void run() {
                ...
                while (!stop) {
                    ...
                    try {
                         ...
                         */
                        if (!validVoter(response.sid)) { // 非法的投票者
    ...
    private void sendNotifications() {
        for (long sid : self.getCurrentAndNextConfigVoters()) {
            ...
            ToSend notmsg = new ToSend(
                ...
                sid,
                proposedEpoch, // 3 这三个参数是updateProposal中设置的
                qv.toString().getBytes(UTF_8));
    ...
    public Vote lookForLeader() throws InterruptedException {
        ...
        try {
            ...
            synchronized (this) {
                ...
                // 更新提议，包含（myid, lastZxid, epoch）该方法的第一个参数是设置投票给那个leader的id,这里直接使用当前节点id,投给自己
             ...
             */
            // 本服务器状态为LOOKING 并且还未选出leader
            while ((self.getPeerState() == ServerState.LOOKING) && (!stop)) {
                /*
                 * Remove next notification from queue, times out after 2 times
                 * the termination time
                 获取其他服务器的投票,从接收队列获取数据
                 */
                ...
                if (n == null) {
                    ...
                    // 因为之前已经sendNotifications过一次了（投给自己）
                    // 所以如果走到这里来，发现有东西没有发送出去
                    // 那么可能的原因就是连接还没建立，还没有和其他节点建立通信，
